...
class UEnc(nn.Module):
    def __init__(self, k, ch_mul, in_chans):
        ...
        self.enc3 = DepthwiseSeparableConv3D(2*ch_mul, 4*ch_mul)
        ...
        self.upconv1 = nn.ConvTranspose3d(16*ch_mul, 8*ch_mul, kernel_size=(4,5,4), stride=2, padding=1)
        self.upconv2 = nn.ConvTranspose3d(8*ch_mul, 4*ch_mul, kernel_size=4, stride=2, padding=1, output_padding=(0,1,0))
        self.upconv3 = nn.ConvTranspose3d(4*ch_mul, 2*ch_mul, kernel_size=(3,9,15), stride=3, padding=2)
        self.upconv4 = nn.ConvTranspose3d(2*ch_mul, ch_mul, kernel_size=20, stride=2, padding=1, output_padding=(1,1,0))
...
class UDec(nn.Module):
    def __init__(self, k, ch_mul, in_chans, out_chans):
        super(UDec, self).__init__()
        
        self.enc1=ConvBlock(k, ch_mul) # , seperable=False
        self.enc2=DepthwiseSeparableConv3D(ch_mul, 2*ch_mul)
        self.enc3=DepthwiseSeparableConv3D(2*ch_mul, 4*ch_mul)
        self.enc4=DepthwiseSeparableConv3D(4*ch_mul, 8*ch_mul, kernel_size=1)
        ...
        self.up1=nn.ConvTranspose3d(16*ch_mul, 8*ch_mul, kernel_size=(4,5,4), stride=2, padding=1)
        
        self.dec1=DepthwiseSeparableConv3D(16*ch_mul, 8*ch_mul, kernel_size=1)
        self.up2=nn.ConvTranspose3d(8*ch_mul, 4*ch_mul, kernel_size=(2,3,2), stride=2)
        
        self.dec2=DepthwiseSeparableConv3D(8*ch_mul, 4*ch_mul, kernel_size=1)
        self.up3=nn.ConvTranspose3d(4*ch_mul, 2*ch_mul, kernel_size=(3,5,11), stride=3, padding=(2,0,0))
        
        self.dec3=DepthwiseSeparableConv3D(4*ch_mul, 2*ch_mul, kernel_size=1)
        self.up4=nn.ConvTranspose3d(2*ch_mul, ch_mul, kernel_size=18, stride=2, output_padding=(1,1,0))
    ...
    def forward(self, x):
        ...
        up4 = torch.cat([enc1, self.up4(dec3)], 1) # exp 129, got 130
        dec4 = self.dec4(up4)
